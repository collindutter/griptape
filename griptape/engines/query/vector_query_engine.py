...
class VectorQueryEngine(BaseQueryEngine):
    ...
    vector_store_driver: BaseVectorStoreDriver = field(default=Factory(lambda: LocalVectorStoreDriver()), kw_only=True)
    prompt_driver: BasePromptDriver = field(default=Factory(lambda: OpenAiChatPromptDriver()), kw_only=True)
    template_generator: J2 = field(default=Factory(lambda: J2("engines/vector_query.j2")), kw_only=True)
